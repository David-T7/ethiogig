...
class ResumeCheckerSerializer(serializers.ModelSerializer):
    class Meta:
        model = models.Interviewer
        fields = [
            'id',
...
class DisputeManagerSerializer(serializers.ModelSerializer):
    class Meta:
        model = models.DisputeManager
        fields = [
            'id',
            'full_name', 
            'email',         
...
class MessageSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'chat', 'sender', 'content','file', 'timestamp','read']
...
class MarkMessagesAsReadSerializer(serializers.Serializer):
    ...
    def validate_message_ids(self, value):
        messages = models.Message.objects.filter(id__in=value)
        if len(messages) != len(set(value)):
            existing_ids = set(messages.values_list('id', flat=True))
            invalid_ids = set(value) - existing_ids
            raise serializers.ValidationError(f"Invalid message IDs: {', '.join(map(str, invalid_ids))}")
        return value
